...
###########################  COPY YOUR LAB6 CODE FOR THIS FUNCTION  ####################################
    ...
    def isSubgraph(self, graph):
        ...
        if self.totalVertices() > graph.totalVertices():
            return False
        
        
        for v1 in range(graph.totalVertices()):
            for v2 in range(v1, graph.totalVertices()):
                if self.getEdge(v1,v2) > 0 and self.getEdge(v1,v2) != graph.getEdge(v1,v2):
                    return False
        return True
